    ...
    render() {
        return(
            <div className='demo'>
                ...
                <input type='text' value={this.state.incrementAmount} onChange={(e) => this.setIncrementAmount(e.target.value)} />

                <button id='increment' onClick={() => this.props.dispatch(incrementByAmount(
                    {amount: this.state.incrementAmount}))}>Add Amount</button>
            </div>
        )
    }
}
